...
for i in range(6):
    ...
    def check_lists(altes_zitat, neues_zitat, wörter_indexe):
        # Verbesserung: Direkter Vergleich der Listen
        return all(altes_wort == neues_wort for altes_wort, neues_wort in zip((wort for wort in altes_zitat if wort != ""), (v for zahl, v in enumerate(neues_zitat) if zahl not in wörter_indexe)))
